...
export function PurchasesPanel({ member }: PurchasesPanelProps) {
  ...
  const handleRequestSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setRequestForm({ title: "", description: "", estimatedCost: "", priority: "medium" });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-semibold text-white">Purchase Management</h3>
      ...
      {showRequestForm && (
        <div className="glass-panel p-6">
          <h4 className="text-lg font-medium text-white mb-4">New Purchase Request</h4>
      ...
      {showOrderForm && (
        <div className="glass-panel p-6">
          <h4 className="text-lg font-medium text-white mb-4">Create Purchase Order</h4>
                    ...
                    <div className="flex space-x-2 ml-4">
                      ...
                      <button
                        onClick={() => {
                          ...
                          handleStatusUpdate(request._id, "rejected", reason || undefined);
                        }}
              ...
              <div key={order._id} className="glass-panel p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-300 mb-3">
                      <div>
                        <span className="text-gray-400">Vendor:</span> {order.vendor}
                      </div>
                      <div>
                        <span className="text-gray-400">Total Cost:</span> ${order.totalCost.toFixed(2)}
                      </div>
                      <div>
                        <span className="text-gray-400">Ordered by:</span> {order.ordererName}
                      </div>
                      <div>
                    ...
                    {order.confirmationImageUrl && (
                      <div className="mb-3">
                        <p className="text-sm text-gray-400 mb-2">Order Confirmation:</p>
                        <img
                ...
                {canManageOrders && !order.confirmationImageId && (
                  <div className="border-t border-white/10 pt-4">
                    <p className="text-sm text-gray-400 mb-2">Upload order confirmation:</p>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => {
                      ...
                      className="text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-cyan-500/20 file:text-cyan-300 hover:file:bg-cyan-500/30"
                    />
                  </div>
                )}
              </div>
